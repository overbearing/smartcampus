...
-(void)createSubViews{
    ...
    _btnView.frame = CGRectMake(0,0,UIWidth,self.btnViewHeight);
    _btnView.showsHorizontalScrollIndicator = NO;
    _btnView.backgroundColor = [UIColor whiteColor];
    _btnView.contentSize = CGSizeMake(UIWidth/3*self.titleArray.count, self.btnViewHeight);
    ...
    for (int i = 0; i < self.titleArray.count; i++)  {
        ...
        btn.frame = CGRectMake(UIWidth/3*i, 0, UIWidth/3, self.btnViewHeight);
    ...
    //3.下划线
    self.indicatorView = [[UIView alloc]initWithFrame:CGRectMake(0, self.indicatororgin, self.indicatorWidth, self.indicatorHeight)];
    self.indicatorView.backgroundColor = self.indicatorColor;
//    self.indicatorView.center = CGPointMake(UIWidth/3, kNavHeight+_btnViewHeight - 0.75);//底部留1
...
//        btn.titleLabel.font = self.titleFont;
    ...
    [UIView animateWithDuration:0.3 animations:^{
        self.indicatorView.center = CGPointMake(sender.center.x, self.indicatorView.center.y);//底部留1
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if (_seletedBtn != btn) {
        [UIView animateWithDuration:0.3 animations:^{
            self.indicatorView.center = CGPointMake(btn.center.x, self.indicatorView.center.y);//底部留1
        }];
...
-(void)setIndicatorWidth:(NSInteger)indicatorWidth{
    ...
    self.indicatorView.frame = CGRectMake(0, self.indicatororgin, indicatorWidth, self.indicatorHeight);
    self.indicatorView.center = CGPointMake(_seletedBtn.center.x, self.indicatorView.center.y);//底部留1
}

-(void)setIndicatorHeight:(NSInteger)indicatorHeight{
    _indicatorHeight = indicatorHeight;
    
    self.indicatorView.frame = CGRectMake(0, self.indicatororgin, self.indicatorWidth, indicatorHeight);
   self.indicatorView.center = CGPointMake(_seletedBtn.center.x, self.indicatorView.center.y);//底部留1
}
